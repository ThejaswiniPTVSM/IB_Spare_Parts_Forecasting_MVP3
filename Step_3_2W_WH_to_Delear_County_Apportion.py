...
# DBTITLE 1,Query and aggregate 2W parts billing
df = spark.sql(f"""
    ...
    FROM (select MaterialNumber as part_id,CONCAT(DealerID, '_', country) AS DealerID_country, cast(BillingDate as date) as LeoDate, cast(YF2_InvoiceQuantity as int) qty, YF2_Invoice_ValueinINR as value_INR from oogib_cin.bumblebee.ib_parts_billing_report 
        WHERE BillingDate between (last_day(add_months(current_date, -7)) + 1) AND (last_day(add_months(current_date, -1))) and industry='2W' and Type = 'Parts' and country<>''
        GROUP BY all) group by all
""").toPandas()

# COMMAND ----------

# DBTITLE 1,Delear Apportion Ratio Calculation
...
# DBTITLE 1,Convert the Pandas DataFrame to a Spark DataFrame
final_df_spark=final_df.toPandas()

# COMMAND ----------

# DBTITLE 1,Read the Forecast
df_forecast = spark.sql(f"""SELECT * from vision_dev.vision.monthly_plan_allocation_2W_base""")
...
# DBTITLE 1,Delear Level Apportion
...
# DBTITLE 1,Output Table Formatting
from datetime import datetime

IB_Spares_2W = final_df.toPandas()

IB_Spares_2W['ABC_FMS'] = IB_Spares_2W['ABC_FMS'].replace({
    'AF': 'A_F',
    'BF': 'B_F',
    'CF': 'C_F',
    'BM': 'B_M',
...
IB_Spares_2W['type'] = 'Forecast'

# Automatically set run_date to 3rd of current month
current_date = datetime.now()
run_date = current_date.replace(day=3).strftime('%Y-%m-%d')
IB_Spares_2W['run_date'] = run_date

print(f"Run date set to: {run_date}")

IB_Spares_2W['DealerCode'] = IB_Spares_2W['DealerID']
...
IB_Spares_2W = pd.merge(IB_Spares_2W, Active_Parts, on='Part_Id', how='inner')

IB_Spares_2W.rename(columns={
    'Forecast_month': 'order_date',
    'DealerCode': 'dealer_id',
    'Part_Id': 'part_id',
    'Country': 'country',
    'Part_Description': 'Material_Description',
    'Forecast_Qty': 'order_qty'
}, inplace=True)

IB_Spares_2W_to_O9 = IB_Spares_2W[[
    'order_date', 'dealer_id', 'country', 'part_id', 'order_qty', 
    'type', 'Material_Description', 'industry', 'ABC_FMS', 'run_date'
]]

# COMMAND ----------

# DBTITLE 1,Write the data to Output Table
from pyspark.sql.functions import col

# Cast order_date to string
spark_Plan_forecast_2W = spark.createDataFrame(IB_Spares_2W_to_O9)

# Write the DataFrame to a Delta table
spark_Plan_forecast_2W.write.mode("overwrite").format("delta").partitionBy("order_date").saveAsTable("vision_dev.vision.monthly_forecast_2W_to_O9")
